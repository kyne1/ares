...
var paddlespan = 80;
var paddlestart = 47; // positive is back neg is front, start at this coordinate and move to front
    ...
    let ta = extend(Ability, {
        ...
        update(unit){
            ...
            if(!unitsMap.has(unit.id+"timer")){
                ...
                //print(unitsMap.get(unit.id+"timer")[1]); //both work and print 0 
                //print(unitsMap.get(unit.id+"timer"))
            }
        ...
        draw(unit){
            ...
            lastpos[1] = new Vec2(unit.x,unit.y);//will be past position becuase only loaded when loop around
            //print("x: "+changepos[0].x);
            //print("y: "+changepos[0].y);
            var spacing = paddlespan/(paddlenum-1);
            paddlemove[0] += Math.sin(toRad(r)-Math.atan2(changepos[0].y,changepos[0].x))*Vec2Len(changepos[0]);
            if(paddlemove[0] > spacing){
                paddlemove[0] -= spacing;
            }
            else if(paddlemove[0] < spacing){
                paddlemove[0] += spacing;
            }
            //print(paddlemoveL);
            ...
            for(let j = -1; j < 2; j+=2){
                for(let i = 0; i < paddlenum; i++){
                    let pos = new Vec2(
                        j*paddlex,
                        spacing*i - paddlestart + paddlemove[0]
                    )
